...
export default function CouponsView({ adminKey }: { adminKey: string }) {
  ...
  const [editForm, setEditForm] = useState<Omit<Coupon, "id" | "active">>({ ...EMPTY });
  ...
  function startEdit(c: Coupon) {
    ...
    setEditForm({
      ...
      type: c.type, discount_type: c.discount_type,
      discount_value: c.discount_value, minimum_spend: c.minimum_spend,
      ...
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-neutral-200 p-8 text-center">
            ...
            <p className="text-[11px] text-neutral-300">Click &quot;Sync Defaults&quot; to create the standard templates, or create a custom one above.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {coupons.map(c => (
              <div key={c.id}
                className={`bg-white rounded-xl border p-4 ${c.active ? "border-neutral-100" : "border-neutral-100 opacity-50"}`}>
                {editingId === c.id ? (
                  <div className="space-y-3">
                    <p className="text-[11px] font-bold text-[#C9A84C] uppercase tracking-[0.1em]">Editing Template</p>
                    <CouponFormFields form={editForm} setForm={setEditForm} />
                    <div className="flex gap-2">
                      <button onClick={saveEdit} disabled={editSaving}
                        className="flex-1 py-2 bg-[#1C1C1C] text-white text-[12px] font-semibold rounded-lg hover:bg-[#C9A84C] hover:text-[#1C1C1C] transition-all disabled:opacity-40">
                        {editSaving ? "Saving…" : "Save Changes"}
                      </button>
                      <button onClick={() => setEditingId(null)}
                        className="px-4 py-2 text-[12px] border border-neutral-200 rounded-lg hover:border-neutral-400 transition-colors">
                        Cancel
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button onClick={() => startEdit(c)}
                        className="text-[10px] font-semibold px-3 py-1.5 rounded-lg border border-neutral-200 hover:border-[#C9A84C] hover:text-[#C9A84C] transition-all">
                        Edit
                      </button>
...
function CouponFormFields({
  ...
  form: Omit<Coupon, "id" | "active">;
  setForm: React.Dispatch<React.SetStateAction<Omit<Coupon, "id" | "active">>>;
}) {
  return (
